...
import { login, initiateGoogleOAuth, initiateGitHubOAuth, processOAuthRedirect } from '../../Redux/Auth/Action'
...
const Login = ({ onForgotPassword }) => {
    ...
    useEffect(() => {
        if (auth?.isOAuthUser && auth?.oauthProvider && auth?.user) {
            // You can add additional logic here for OAuth users
            // e.g., different welcome message, redirect logic, etc.
        }
    }, [auth?.isOAuthUser, auth?.oauthProvider, auth?.user]);
    ...
    // Dynamic button text based on loading states
    const getButtonText = () => {
    ...
    return (
        <div className='space-y-5'>
            ...
            {/* Forgot Password Link - Hide for OAuth users */}
            {onForgotPassword && !auth?.isOAuthUser && (
